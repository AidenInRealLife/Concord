...
const state = {
  ...
  voice: {
    ...
    participants: new Map(), // socketId -> { userId, username, color, avatarUrl }
    analysers: new Map(),    // socketId ('me' incluso) -> { analyser, dataArray }
    speakingCheckInterval: null,
    screenShareSocketId: null, // quem está exibido no palco de tela compartilhada agora
  },
};
...
const SCREEN_QUALITY_PRESETS = {
  high: { width: { ideal: 1920, max: 1920 }, height: { ideal: 1080, max: 1080 }, frameRate: { ideal: 60, max: 60 } },
  medium: { width: { ideal: 1280, max: 1280 }, height: { ideal: 720, max: 720 }, frameRate: { ideal: 30, max: 30 } },
  low: { width: { ideal: 854, max: 854 }, height: { ideal: 480, max: 480 }, frameRate: { ideal: 15, max: 15 } },
};
...
function createPeerConnection(socketId, isInitiator) {
  ...
  pc.ontrack = (e) => {
    if (e.track.kind === 'video') {
      ...
      attachRemoteAudio(socketId, e.streams[0]);
...
function renderVoiceGrid() {
  ...
  const meTile = buildVoiceTile('me', { username: state.user.username + ' (você)', color: state.user.color, avatarUrl: state.user.avatarUrl });
...
function ensureSpeakingLoop() {
  ...
  state.voice.speakingCheckInterval = setInterval(() => {
    state.voice.analysers.forEach((entry, key) => {
      ...
      if (tile) tile.classList.toggle('speaking', avg > 14);
    });
...
$('#shareScreenBtn').addEventListener('click', async () => {
  ...
  const constraints = SCREEN_QUALITY_PRESETS[quality] || SCREEN_QUALITY_PRESETS.medium;

  try {
    state.voice.screenStream = await navigator.mediaDevices.getDisplayMedia({ video: constraints, audio: false });
  } catch (e) {
    return; // usuário cancelou a seleção de tela
  }

  const track = state.voice.screenStream.getVideoTracks()[0];
...
function stopScreenShare() {
  ...
  state.voice.screenStream.getTracks().forEach(t => t.stop());
